...
def quantize_gptq(model_path: str, output: str, bits: int, group_size: int,
    ...
    if os.path.isdir(local_path):
        ...
        calib = ds.select(range(min(nsamples, len(ds))))["text"]
        print(f"  从 PVC 加载: {local_path}")
        ...
        calib = sample_texts[:min(nsamples, len(sample_texts))]
        print(f"  使用内嵌校准样本 ({len(calib)} 条)")

    if calib is None:
        from datasets import load_dataset
        raw = load_dataset(dataset, split="train", trust_remote_code=True)
        calib = raw.select(range(nsamples))["text"]
