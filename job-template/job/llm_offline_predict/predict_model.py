...
class Offline_Predict():
    ...
    def callback(self, ch, method, properties, body):  # 四个参数为标准格式
        ...
        # 先 ack 再处理，防止处理过程中连接断开导致消息 redeliver 到另一个 worker，
        # 两个 worker 各写一次结果文件造成重复输出
    ...
    def start_predict(self, local_rank):
        ...
        # 分布式启动方式
            ...
            # 只用 ROLE 环境变量判断（两个 Task 的 VC_TASK_INDEX 各自从 0 开始编号，不能混用）
            if ROLE == 'master':
                # 等待 RabbitMQ 就绪（Pod 可能先于 RabbitMQ 启动）
                self._wait_rabbitmq()
                self.producter()
                self.wait_finish()
                # 生产者完成后不能退出，否则 Volcano 任务 minAvailable
                # 不满足 (通常 auto-set 为 replicas=3)，会触发 AbortJob
                print('Producer finished, staying alive to keep task healthy...', flush=True)
                while True:
                    time.sleep(60)
            else:
                self.consumer()

    def _wait_rabbitmq(self, max_retries=30, interval=2):
        """等待 RabbitMQ 就绪"""
        rabbit_host = os.environ.get('RABBIT_HOST', '127.0.0.1')
        for i in range(max_retries):
            try:
                rabbit_client = Rabbit_info(host=rabbit_host)
                rabbit_client.get_msg_count()
                print(f'RabbitMQ ready (attempt {i+1})', flush=True)
                return
            except Exception as e:
                print(f'Waiting for RabbitMQ {rabbit_host} (attempt {i+1}/{max_retries}): {e}', flush=True)
                time.sleep(interval)
        print(f'RabbitMQ still not ready after {max_retries} attempts, proceeding anyway...', flush=True)
