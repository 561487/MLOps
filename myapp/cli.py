    ...
    if conf.get('BABEL_DEFAULT_LOCALE','zh')=='zh':
        try:
            ...
            if SQLALCHEMY_DATABASE_URI:
                ...
                if len(tables)==0:
                    db.session.add(Metadata_table(app='mlops', db=database, table='project', owner='admin',describe='项目分组，模板分组，模型分组'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='project_user', owner='admin',describe='项目组用户'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='idex_query', owner='admin',describe='sqllab的查询记录'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='metadata_table', owner='admin',describe='离线库表管理'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='metadata_metric', owner='admin',describe='指标管理'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='dimension', owner='admin',describe='维表管理'))
                    db.session.add(Metadata_table(app='mlops',db=database,table='dataset',owner='admin',describe='数据集市场'))

                    db.session.add(Metadata_table(app='mlops', db=database, table='repository', owner='admin',describe='docker仓库管理'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='docker', owner='admin', describe='在线docker镜像构建'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='images', owner='admin',describe='镜像管理'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='notebook', owner='admin', describe='notebook在线开发'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='etl_pipeline', owner='admin',describe='数据ETL的任务流管理'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='etl_task', owner='admin',describe='数据ETL的任务管理'))

                    db.session.add(Metadata_table(app='mlops', db=database, table='job_template', owner='admin',describe='任务模板'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='pipeline', owner='admin',describe='ml任务流'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='task', owner='admin', describe='ml任务管理'))

                    db.session.add(Metadata_table(app='mlops', db=database, table='run', owner='admin',describe='定时调度记录'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='workflow', owner='admin',describe='任务流实例'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='nni', owner='admin', describe='nni超参搜索'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='service', owner='admin',describe='内部服务管理'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='model', owner='admin', describe='模型管理'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='inferenceservice', owner='admin', describe='推理服务'))

                    db.session.add(Metadata_table(app='mlops',db=database,table='aihub',owner='admin',describe='模型应用市场，打通自动化标注，一键开发，一键微调，一建部署'))
                    db.session.add(Metadata_table(app='mlops',db=database,table='chat',owner='admin',describe='私有知识库，配置领域知识文档或qa文档，智能机器人问答'))
                    db.session.add(Metadata_table(app='mlops',db=database,table='chat_log',owner='admin',describe='所有的聊天日志记录'))
                    db.session.add(Metadata_table(app='mlops', db=database, table='favorite', owner='admin',describe='收藏的数据记录'))

                    db.session.add(Metadata_table(app='mlops', db=database, table='logs', owner='admin',describe='用户行为记录'))
                    db.session.commit()
                    print('添加离线表成功')
